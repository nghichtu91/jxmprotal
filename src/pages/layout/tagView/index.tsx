...
const TagsView: FC = () => {
  const { tags, activeTagId } = useSelector((state: AppState) => state.tagsView);
  const { menuList } = useSelector((state: AppState) => state.user);
  ...
  return (
    <div id="pageTabs" style={{ padding: '6px 4px' }}>
      ...
      >
        {tags.map(tag => (
          <TabPane tab={tag.label.en_US} key={tag.path} closable={tag.closable} />
        ))}
      </Tabs>
    </div>
  );
