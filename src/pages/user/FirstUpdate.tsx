...
const FisrtUpdatePage: FC = () => {
  ...
  const { username, newUser, phone, email } = useSelector((s: AppState) => s.user);
      ...
      <Card>
        <Form<IUserUpdateDTO> layout="vertical" onFinish={onFinish} labelAlign="right" className="change-info-form">
          ...
          <Form.Item
            ...
            name="phone"
            rules={[
              { required: !phone, message: 'Vui lòng nhập số điện thoại.' },
              {
          ...
          <Form.Item
            hidden={!email}
            tooltip="Vui lòng nhập email bạn đã từng đăng ký, nếu quên liên hệ gm."
            name="email"
            label={<Typography.Text strong>Email</Typography.Text>}
            rules={[{ required: !!email, message: 'Vui lòng nhập email.' }]}
          >
