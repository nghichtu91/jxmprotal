...
class UserRepository implements IUserRepository {
  ...
  async adminPaymentHistories<T>(params: T): Promise<PageData<IPaymentDTO>> {
    try {
      const { data } = await this.axiosInstance.get<PageData<IUserDto>>(`payments/admin/histories`, {
        params: params,
      });
  ...
  async smsRequest(params: ISmsCreateParams, action: SmsActions): Promise<string> {
    try {
      if (action === 'forgotpass') {
        const { data } = await this.axiosInstance.post<IBaseResponse>(`sms/sms-request-password-forgot`, params);
  ...
  async adminActions<T>(params: T, username: string, action: AdminActions): Promise<boolean> {
    try {
      await this.axiosInstance.patch(`admin/users/${username}/${action}`, params);

      return true;
    } catch (error) {
      let errorCode: string | undefined = `EXCEPTION_ADMIN_${action.toUpperCase()}_ERRORS`;
  ...
  async logout(): Promise<boolean> {
    this.removeToken();
    await this.httpClient.removeToken();
